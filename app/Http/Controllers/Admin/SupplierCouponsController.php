...
class SupplierCouponsController extends Controller
{
	...
	{
		$vendor_coupans = VendorCoupons::supplier()->leftjoin('vendor_coupons_useds','vendor_coupons_useds.coupon_id','vendor_coupons.id')
							->join('vendors','vendors.id','=','vendor_coupons.vender_id')
								...
								->get();
		return view('admin/supplier_coupons/index',compact('vendor_coupans'));
	}
	...
	*/
	public function create()
	{
		...
		$vendor_stores = VendorStore::supplier()->get();
		$users = User::all();
		return view('admin/supplier_coupons/create',compact('vendors','vendor_stores','users'));
	}
	...
	*/
	public function store(Request $request)
	{
		...
		return redirect('/admin/supplier_coupons')->with('success',"Coupon has been saved.");
	}
	...
	*/
	...
	{
		$vendor_coupon = VendorCoupons::supplier()->find($id);
		if($vendor_coupon->status == 'enable'){
			VendorCoupons::supplier()->where('id',$id)->update(array('status' => 'disable'));
			echo json_encode('disable');
		}else{
			VendorCoupons::supplier()->where('id',$id)->update(array('status' => 'enable'));
	...
	*/
	public function edit(VendorCoupons $vendor_coupon)
	{
		...
		return view('admin/supplier_coupons/edit',compact('vendors','vendor_stores','vendor_coupon','vendor_categories','users','discount_users'));
	}
	...
	*/
	public function update(Request $request, $id)
	{
		...
		if ($files = $request->file('image')){
			$path = 'public/images/suppliers_coupons';
			$profileImage = date('YmdHis') . "." . $files->getClientOriginalExtension();
			$files->move($path, $profileImage);
			$data['image'] = $profileImage;
		}

		VendorCoupons::supplier()->where('id',$id)->update($data);
		...
		return redirect('/admin/supplier_coupons')->with('success',"Coupon has been updated.");
	}
	...
	*/
	public function destroy(VendorCoupons $vendor_coupon)
	{
		...
		return redirect('/admin/supplier_coupons')->with('success',"Coupon has been deleted.");
	}
	...
	public function verifiedVendorCoupon($id)
	{
		...
		VendorCoupons::supplier()->where('id',$id)->update($data);
		return redirect('/admin/supplier_coupons/unverified')->with('success',"Coupon has been Verified.");
	}

	public function verifiedEditVendorCoupon($id)
	{
		$vendor_coupon = VendorCoupons::supplier()->where('id',$id)->first();
		$vendors = Vendor::supplier()->where('parent_id',0)->get();
		$vendor_stores = VendorStore::supplier()->get();
		$vendor_categories = Category::all();
		$users = User::where('status','active')->get();
        $discount_users = UserCoupon::where('coupon_id',$id)->pluck('user_id')->toArray();
		return view('admin/supplier_coupons/unverified_edit',compact('vendors','vendor_stores','vendor_coupon','vendor_categories','users','discount_users'));
	}
}
