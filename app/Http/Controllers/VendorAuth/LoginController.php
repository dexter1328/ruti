...
class LoginController extends Controller
{
    ...
    */
    ...
    public function showLoginForm()
    {
        ...
        return view('vendor.auth.login',compact('wb_wishlist'));
    }
    ...
    public function login(Request $request)
    {
        ...
        if(!empty($user)){
            if ($user && \Hash::check($request->password, $user->password)) {
                if($user->parent_id!=0){
                    $parent_user = Vendor::where('id',$user->parent_id)->first();
        ...
        return redirect('/vendor/login')->withErrors($errors);
    }
    ...
    public function submitOTP(Request $request, $token)
    {
        ...
        if(empty($request->all())){
            return view('vendor/auth/otp', compact('vendor_otp', 'time_diff'));
        }else{
            ...
            if(!empty($chk_vendor_otp))
            {
                ...
                if($diff_in_minutes <= 15)
                {
                    ...
                    Helper::addToLog('Vendor Login',$chk_vendor_otp->id);
                    return redirect('vendor/home');
                }else{
