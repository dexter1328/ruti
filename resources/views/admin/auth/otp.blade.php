...
<div class="card-body">
    <div class="card-content p-2">
        ...
        <form class="form-horizontal" role="form" method="POST" action="{{ url('/admin/submit-otp/'.$admin_otp->token) }}">
            {{ csrf_field() }}
...
<script type="text/javascript">
    ...
    $( document ).ready(function() {
        $("#resend_otp").click(function(){
            $.ajax({
                type: "post",
                data: {email: '{{$admin_otp->email}}', token: '{{$admin_otp->token}}'},
                url: "{{ url('/admin/resend_otp_mail') }}",
                headers: {
                ...
                success: function (data) {
                    ...
                    //$(".error").html('');
                    window.location.replace("{{ url('admin/submit-otp/'.$admin_otp->token) }}");
                }
            });
        });
    });
