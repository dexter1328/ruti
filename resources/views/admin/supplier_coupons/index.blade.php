	...
	<div class="col-lg-12">
		<div class="card">
			<div class="card-header">
				<div class="left"><!-- <i class="fa fa-tags"></i> --><span>Supplier Coupons</span></div>
				<div class="float-sm-right">
					<a style="padding-bottom: 3px; padding-top: 4px;" href="{{ route('supplier_coupons.create') }}" class="btn btn-outline-primary btn-sm waves-effect waves-light m-1" title="Add Coupon">
						<!-- <i class="fa fa-tag" style="font-size:15px;"></i> --> <span class="name">Add Coupon</span>
					</a>
				</div>
			</div>
			<div class="card-body">
							...
							@foreach($vendor_coupans as $key=> $vendor_coupan)
								<tr>
									...
									@if($vendor_coupan->count_used_coupon != 0)
                                    	<td><a href="JavaScript:void(0);" onclick="viewUsedCoupon('{{$vendor_coupan->id}}')" style="color:#424f9b;font-weight: 600;">							{{$vendor_coupan->count_used_coupon}} Order </a>
                                    	</td>
                                	@else
                                    	<td>−</td>
                                	@endif
									<td class="action">
										<form id="deletefrm_{{$vendor_coupan->id}}" action="{{ route('supplier_coupons.destroy', $vendor_coupan->id) }}" method="POST" class="delete"  onsubmit="return confirm('Are you sure?');">
											@csrf
											@method('DELETE')
											<a href="{{ route('supplier_coupons.edit', $vendor_coupan->id) }}" class="edit" data-toggle="tooltip" data-placement="bottom" title="Edit Coupon">
											...
											<a href="javascript:void(0);" onclick="changeStatus('{{$vendor_coupan->id}}')" >
									 			<i class="fa fa-circle status_{{$vendor_coupan->id}}" style="@if($vendor_coupan->status=='enable')color:#009933;@else color: #ff0000;@endif" id="enable_{{$vendor_coupan->id}}" data-toggle="tooltip" data-placement="bottom" title="Change Status"></i>
											</a>
										</form>
									</td>
								</tr>
    ...
    <div class="modal-dialog">
        ...
        <div class="modal-content schedule-booking">
            ...
            <div class="modal-body" id="assign-modal">
              <table class="table table-bordered table-striped" id="discount_code_table" >
                  <thead>
                      <tr>
                            <td>Customer</td>
                      </tr>
...
function changeStatus(id){
	$.ajax({
		...
		url: "{{ url('admin/vendor_coupons/coupon_status') }}/"+id,
		type: "GET",
		dataType: 'json',
...
{
    $.ajax({
        url: "{{url('admin/vendor_coupons/view_used_coupon')}}/"+id,
        type: "GET",
        dataType: 'html',
        success: function (data) {
