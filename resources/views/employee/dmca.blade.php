    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-header">
                <div class="left"><span>DMCA</span></div>
            </div>
            <div class="card-body">
            	<div class="container-fluid">
                    ...
                    @endif
                    <div id="ajaxMsg"></div>
                    @php //echo '<pre>'; print_r($dmca['content']); echo '</pre>'; @endphp
                    <form id="addFrm" method="post" action="{{route('pagemeta.dmca')}}" enctype="multipart/form-data"> 
                        @csrf 
                        <div class="form-group">
                            <label for="title">Title</label>
                            <input type="text" class="form-control" id="title" name="title" value="{{old('title', isset($dmca['title']) ? $dmca['title'] : '')}}"> 
                            @if ($errors->has('title')) 
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control" id="description" name="description" rows="10">{{old('description', isset($dmca['description']) ? $dmca['description'] : '')}}</textarea>
                        ...
                        <div class="form-group">
                            <input type="hidden" id="total_content" value="{{isset($dmca['content']) ? count($dmca['content']) : 1}}">
                            <table class="table table-bordered" width="100%">
                                ...
                                <tbody id="value_rows">
                                    @foreach($dmca['content'] as $key => $content)
                                    <tr>
                                        <td>
                                            <input type="text" name="dmca[{{$key}}][title]" class="form-control" value="{{$content->title}}">
                                        </td>
                                        <td>
                                            <textarea name="dmca[{{$key}}][description]" class="form-control" rows="5">{{$content->description}}</textarea>
                                        </td>
                                        <td class="action">
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $('#add_value').click(function(){
            ...
            row += '<tr>';
                row += '<td>';
                    row += '<input type="text" name="dmca['+values_counter+'][title]" class="form-control" value="">';
                row += '</td>';
                row += '<td>';
                    row += '<textarea name="dmca['+values_counter+'][description]" class="form-control" rows="5"></textarea>';
                row += '</td>';
                row += '<td>';
    ...
    function removeContent(e, id)
    {
        ...
        if (r == true) {
            $.ajax({
                type: "GET",
                url: "{{ url('employee/remove-dmca') }}/"+id,
