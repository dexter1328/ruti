		...
		<div class="card">
			<div class="card-header">
				...
				<div class="float-sm-right">
					<a style="padding-bottom: 3px; padding-top: 4px;" href="{{ route('supplier.customer_feedback.create') }}" class="btn btn-outline-primary btn-sm waves-effect waves-light m-1" title="Add Feedback">
						...
						<tbody>
							@foreach($customer_feedbacks as $key=>$customer_feedback)
								<tr>
									...
									<td class="action">
										<form id="deletefrm_{{$customer_feedback->id}}" action="{{ route('supplier.customer_feedback.destroy', $customer_feedback->id) }}" method="POST" class="delete"  onsubmit="return confirm('Are you sure?');">
											@csrf
											@method('DELETE')
											<a href="{{ route('supplier.customer_feedback.edit', $customer_feedback->id) }}" class="edit">
...
function changeStatus(id) {
    $.ajax({
        ...
        url: "{{ url('supplier/customer_feedback') }}/"+id,
        type: "GET",
        dataType: 'json',
