		...
		<div class="card">
			<div class="card-header">
				...
				<div class="float-sm-right">
					<a style="padding-bottom: 3px; padding-top: 4px;" href="{{ route('supplier.customer_reviews.create') }}" class="btn btn-outline-primary btn-sm waves-effect waves-light m-1" title="Add New Review">
						...
						<tbody>
							@foreach($customer_reviews as $key=> $customer_review)
								<tr>
									...
									<td class="action">
										<form id="deletefrm_{{$customer_review->id}}" action="{{ route('supplier.customer_reviews.destroy', $customer_review->id) }}" method="POST" class="delete"  onsubmit="return confirm('Are you sure?');">
											@csrf
											@method('DELETE')
											<a href="{{ route('supplier.customer_reviews.edit', $customer_review->id) }}" class="edit">
...
function changeStatus(id) {
    $.ajax({
        ...
        url: "{{ url('supplier/customer_reviews') }}/"+id,
        type: "GET",
        dataType: 'json',
