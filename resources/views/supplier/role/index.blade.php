    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-header">
                <div class="left"><span>Supplier Roles</span></div>
                <!-- <div class="float-sm-right"> <a style="padding-bottom: 3px; padding-top: 4px;" href="{{ route('supplier.supplier_roles.create') }}" class="btn btn-outline-primary btn-sm waves-effect waves-light m-1" title="Add Role"> <span class="name">Add Role</span> </a> </div> -->
            </div>
            <div class="card-body">
                    ...
                    <table id="example" class="table table-bordered">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="action">
                                    <form id="deletefrm_{{$vendor_role->id}}" action="{{ route('supplier.supplier_roles.destroy', $vendor_role->id) }}" class="delete" method="POST" onsubmit="return confirm('Are you sure?');">
                                        @csrf
                                        @method('DELETE')
                                        <a href="{{ route('supplier.supplier_roles.edit', $vendor_role->id) }}" class="edit" data-toggle="tooltip" data-placement="bottom" title="Edit Role">
...
function changeStatus(id) {
    $.ajax({
        ...
        url: "{{ url('supplier/supplier_roles') }}/"+id,
        type: "GET",
        dataType: 'json',
...
function viewRoles(id)
{
    ...
    var html ='';
    $.ajax({
        url:'{{ url("supplier/supplier_roles/show") }}/'+id,
        type: "GET",
        dataType: 'html',
        success: function (data) {
            ...
            $.each(modal_data, function( key, value ) {
                var url = '{{ url("supplier/supplier_roles/delete") }}/'+value.id;
...
{
    $.ajax({
        url:'{{ url("supplier/supplier_roles/delete") }}/'+id,
        type: "GET",
        dataType: 'html',
        success: function (data) {
