						...
						<tbody>
							@foreach($vendor_coupans as $key => $vendor_coupan)
								<tr>
									...
									@php /* @endphp
									<td class="action">
										<form id="deletefrm_{{$vendor_coupan->id}}" action="{{ route('supplier.supplier_coupons_used.destroy', $vendor_coupan->id) }}" method="POST" class="delete"  onsubmit="return confirm('Are you sure?');">
										@csrf
										@method('DELETE')
										<a href="{{ route('supplier.supplier_coupons_used.edit', $vendor_coupan->id) }}" class="edit">
