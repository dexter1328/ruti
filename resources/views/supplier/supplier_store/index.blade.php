		...
		<div class="card">
			<div class="card-header">
					...
					<div class="col-9">
						<div class="store-btn-right">
		                	<div class="row">
		                        ...
		                        <div class="col-xs-12 col-sm-3">
		                      		<a href="{{ url('supplier/stores/export/store') }}" class="btn btn-outline-primary btn-sm waves-effect waves-light m-1"><span class="name">Export</span></a>
		                        </div>
		                        <div class="col-xs-12 col-sm-3">
		                        	<a href="{{ route('supplier.stores.create') }}" class="btn btn-outline-primary btn-sm waves-effect waves-light m-1" title="Add Store">
				...
				<div class="float-sm-right">
					<a style="padding-bottom: 3px; padding-top: 4px;" href="{{ route('supplier.stores.create') }}" class="btn btn-outline-primary btn-sm waves-effect waves-light m-1" title="Add Store">
						...
						<tbody>
							@foreach($vendor_stores as $key=>$vendor_store)
								<tr>
									...
									<td class="action">
										<form id="deletefrm_{{$vendor_store->id}}" action="{{ route('supplier.stores.destroy', $vendor_store->id) }}" method="POST" class="delete"  onsubmit="return confirm('Are you sure?');">
											@csrf
											@method('DELETE')
											<a href="{{ route('supplier.stores.edit', $vendor_store->id) }}" class="edit" data-toggle="tooltip" data-placement="bottom" title="Edit Store">
...
<div class="modal fade" id="importModal">
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div id="error"></div>
				<form method="post" action="{{ url('supplier/stores/import-store') }}" enctype="multipart/form-data">
					@csrf
