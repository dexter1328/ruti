...
<style type="text/css">
	...
	div#editCardModal {width: 30%;margin: 0 auto;}
</style>
...
<div class="row">
		...
		<div class="card card-small mb-4 pt-3">
			<div class="card-header border-bottom text-center">
				<div class="mb-3 mx-auto">
					@if($vendor->image)
						@php $image = asset('public/images/suppliers/'.$vendor->image); @endphp
	...
	<div class="col-lg-8">
		<div class="card">
			...
			<div class="card-body">
				<form id="signupForm" method="post" action="{{ url('supplier/profile') }}" enctype="multipart/form-data">
					@csrf
					...
					<center>
						<div class="form-footer">
							...
							<a href="{{url('supplier/home')}}"><button type="button" class="btn btn-danger"><i class="fa fa-times"></i> CANCEL</button></a>
						</div>
					</center>
				</form>
			</div>
...
function updateCard(e) {
	...
	$.ajax({
		type: "POST",
		url: "{{url('/supplier/edit-supplier-card/')}}/",
		headers: {
			'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
		},
...
function deleteCard(cid) {
	...
	if (confirm("Are you sure!") == true) {
		...
		$.ajax({
			type: "POST",
			url: "{{url('/supplier/delete-supplier-card/')}}/",
			headers: {
				'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
			},
			data: {card_id: cid},
...
function setDefaultCard(cid) {
	...
	$.ajax({
		type: "POST",
		url: "{{url('/supplier/set-supplier-default-card/')}}/",
		headers: {
			'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
		},
		data: {card_id: cid},
