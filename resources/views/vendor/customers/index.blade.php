		...
		<div class="card">
			<div class="card-header">
				...
				<div class="float-sm-right">        
					<a style="padding-bottom: 3px; padding-top: 4px;" href="{{ route('vendor.customer.create') }}" class="btn btn-outline-primary btn-sm waves-effect waves-light m-1" title="Add Customer">
					...
					<table id="example" class="table table-bordered">
						...
						<tbody>
							...
							@foreach($customers as $key=> $customer)
								<tr>
									<td>{{$key+1}}</td>
									<td><a href="{{ url('vendor/customer/view',$customer->user_id)}}" >{{$customer->first_name}}</a></td>
									@if(isset($vendor_paid_module->status) && $vendor_paid_module->status == 'yes')
									...
									@if(isset($vendor_paid_module->status) && $vendor_paid_module->status == 'yes')
									<td class="action">
										<form id="deletefrm_{{$customer->user_id}}" action="{{ route('vendor.customer.destroy', $customer->user_id) }}" method="POST" class="delete"  onsubmit="return confirm('Are you sure?');">
											@csrf
											@method('DELETE')
											<a href="{{ route('vendor.customer.edit', $customer->user_id) }}" class="edit" data-toggle="tooltip" data-placement="bottom" title="Edit">
...
function changeStatus(id) {
    $.ajax({
        ...
        url: "{{ url('vendor/customer/customer-status') }}/"+id,
        type: "GET",
        dataType: 'json',
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $.each(modal_data, function( key, value ) {
            	var url = '{{ url("vendor/orders/view_order") }}/'+value.id;
