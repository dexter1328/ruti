...
<div class="container-fluid">
	<div class="col-xs-12">
        <form class="date_range" method="post" action="{{url('/vendor/home')}}">
            @csrf
            <div class="row">
                ...
                <div class="col-xs-12 col-md-2">
                    <select class="form-control" name="store" id="store">
                    	...
                    	@foreach($vendor_store as $store_data)
                    		<option value="{{$store_data->id}}"{{(isset($input['store']) && $input['store']==$store_data->id ? 'selected': '')}}>{{$store_data->name}}</option>
                    	@endforeach
                	</select>
                </div>
		...
		<div class="col-12 col-lg-6 col-xl-3">
			<div class="card gradient-deepblue">
				<div class="card-body">
					...
					<a href="{{url('vendor/vendors')}}"><p class="mb-0 text-white small-font">Total Vendors</p></a>
				</div>
			</div> 
		</div>
		<div class="col-12 col-lg-6 col-xl-3">
			<div class="card gradient-orange">
				<div class="card-body">
					...
					<a href="{{url('vendor/stores')}}"><p class="mb-0 text-white small-font">Total Stores</p></a>
				</div>
			</div>
		</div>
		<div class="col-12 col-lg-6 col-xl-3">
			<div class="card gradient-ohhappiness">
				<div class="card-body">
					...
					<a href="{{url('vendor/orders')}}"><p class="mb-0 text-white small-font">Total Orders</p></a>
				</div>
			</div>
		</div>
		<div class="col-12 col-lg-6 col-xl-3">
			<div class="card gradient-ibiza">
				<div class="card-body">
					...
					<a href="{{url('vendor/customer')}}"><p class="mb-0 text-white small-font">Total Users</p></a>
				</div>
			</div>
		</div>
	</div><!--End Row-->
					...
					<div class="row">
						...
						<div class="col col-lg-10">
							...
							<!-- 	<div class="col-lg-4">
								...
								<div class="col-lg-12">
									<a href="{{url('vendor/orders')}}"><button class="btn btn-block btn-primary">View All</button></a>
								</div>
							</div>
						</div>
					</div>
			...
			<div class="card">
				<div class="card-header">Stores Selling
					<div class="card-action">
						<h3 class="box-title" style="float:right;"><a href="{{url('vendor/stores')}}"><button class="btn btn-block btn-primary">View All</button></a></h3>
					</div>
				</div>
				<div class="card-body">
		...
		<div class="col-12 col-lg-12 col-xl-12">
			<div class="card">
				<div class="card-header">Customer Review
					<div class="card-action">
						<h3 class="box-title" style="float:right;"><a href="{{url('vendor/product_reviews')}}"><button class="btn btn-block btn-primary">View All</button></a></h3>
					</div>
				</div>
		...
		<div class="col-12 col-lg-12">
			<div class="card">
				<div class="card-header border-0">Recent Order Tables
					<div class="card-action">
						<h3 class="box-title" style="float:right;"><a href="{{url('vendor/orders')}}"><button class="btn btn-block btn-primary">View All</button></a></h3>
					</div>
				</div>
				<div class="table-responsive">
				...
				<div id="carouselExampleSlidesOnly" class="carousel slide" data-interval="false">
					<div class="carousel-inner">
						<div class="carousel-item active">
							...
							@endif
							<br><br>
							Click <a href="{{url('vendor/profile')}}">here</a> to complete your Profile.
						</div>
						@if(Auth::user()->parent_id == 0)
							<div class="carousel-item">
								...
								<br><br>
								Click <a href="{{url('vendor/choose-plan')}}">Choose Plan</a>
								<br><br>
								Another incentive-15% off when you pay for annual service.
							</div>
							<div class="carousel-item">
								We need to remit funds to your business bank account on the same day. If you haven’t already done so, click <a href="{{url('vendor/profile')}}">bank info</a> to complete your bank name, routing and account.
							</div>
							<div class="carousel-item">
								Great job! Now let’s add your business details, business hours and locations.
								<br><br>
								Click <a href="{{url('vendor/stores')}}">Locations</a> to Add or Import(for multiple stores) all your locations. Please use the download sample CSV for the import format. You can also view the details to ensure you have the store name, store manager and store manager’s email contact correctly entered.
								<br><br>
								Click on <a href="{{url('vendor/vendors')}}">Vendor details</a> to complete form. You can also import multiple vendors if you have subsidiaries.
							</div>
							<div class="carousel-item">
						...
						@elseif(Auth::user()->parent_id != 0 && Auth::user()->vendorRole()->exists())
							...
							@if(Auth::user()->vendorRole->slug == 'store-manager')
								<div class="carousel-item">
									...
									<br><br>
									1. Click <a href="{{url('vendor/stores')}}">store hours</a> to include business schedule. 
									<br><br>
									2. Click <a href="{{url('vendor/vendor_roles')}}">role/permission</a> to add staff and assign specific roles. Assign permission for website and mobile app accessibility.
									<br><br>
									3. Before we proceed, complete your <a href="{{url('vendor/profile')}}">profile</a> as a manager, and start the process for your employees who will use the app. We need one for a supervisor, security person, and floor clerk.
									<br><br>
									4. Now let’s setup your <a href="{{url('vendor/push_notifications')}}">notifications</a>.
								</div>
								<div class="carousel-item">
									Patience wins the crown. We are almost there. Wait, add more things to optimize your business
									<br><br>
									Click <a href="{{url('vendor/categories')}}">category</a> to specify which area you would classify your product.
									<br><br>
									Click <a href="{{url('vendor/attributes')}}">attribute</a> to specify what you want customers to know about your business.
									<br><br>
									Click <a href="{{url('vendor/brand')}}">brand</a> to add information that will increase customer awareness of your brand/business.
									<br><br>
									Click to <a href="{{url('vendor/vendor_coupons')}}">add coupons</a>. Coupons you post monthly to customers depends on the plan you choose. You can skip this point and add on later. 
								</div>
								<div class="carousel-item">
									Click on <a href="{{url('vendor/products')}}">ADD inventory</a> at this point. Download sample CSV for the import format. Also click <a href="{{url('vendor/settings')}}">HERE</a> to set up inventory update reminder.
								</div>
								<div class="carousel-item">
...
<script>
	function changeUserGuide()
	{
		$.ajax({
			type: "post",
			url: "{{ url('/vendor/complete-user-guide/'.Auth::user()->id) }}",
			headers: {
	...
	$("#btn_save").click(function(){
		...
		if($("#bank_routing_no").val() != '' && $("#bank_account_no").val() != '' && $("#bank_name").val() != ''){
			$.ajax({
			    type: "post",
			    url: "{{ url('/vendor/vendors/add_bank_detail') }}",
			    headers: {
			        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
			    },
