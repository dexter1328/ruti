...
<div class="row">
	<div class="col-lg-12">
		<div class="card">
			...
			<div class="card-body">
				<form id="signupForm" method="post" action="{{ route('vendors.update',$vendor->id) }}" enctype="multipart/form-data">
					@csrf
					@method('PATCH')
					...
					<div class="form-group row">
                        ...
                        <div class="col-sm-4">
                        	<input type="hidden" name="store_main_id" value="{{isset($store_vendor->id) ? $store_vendor->id : ''}}">
                            <select id="store_id" name="store_id" class="form-control">
                                <option value="">Select Store</option>
                                @php 
                            		$store_id = '';
                            		if(isset($store_vendor->store_id)) {
                            			$store_id = $store_vendor->store_id;
                            		}
                            	@endphp
                                @if($vendor_stores->isNotEmpty()) 
                                	@foreach($vendor_stores as $vendor_store)
                                	<option value="{{$vendor_store->id}}" {{ (old("store_id", $store_id) == $vendor_store->id ? "selected":"") }}>{{$vendor_store->name}}</option>
                                	@endforeach
                                @endif
                            </select>
					...
					<div class="form-group row">
                        ...
                        <div class="col-sm-4">
                            ...
                            @if($vendor->image)
							<br>
							<a href="{{url('public/images/vendors/'.$vendor->image)}}" rel="prettyPhoto">
								<img src="{{url('public/images/vendors/'.$vendor->image)}}" style="width: 100px; height: auto;">
							</a>
							@endif
                        </div>
					</div>
...
var store_id = "{{old('store_id', $store_id)}}";
